...
(function() {
  ...
  TigerDB = (function(_super) {
    ...
    mapTables = function(row) {
      if (row.name.match(/^[A-Z]/)) return mapTable(row.name);
    };

    TigerDB.prototype.executeQuery = function(string, args, callback) {
      var i, resultArray, resultSet, row, _ref;
      ...
      if (resultSet) {
        while (resultSet.isValidRow()) {
          row = {};
          for (i = 0, _ref = resultSet.fieldCount - 1; 0 <= _ref ? i <= _ref : i >= _ref; 0 <= _ref ? i++ : i--) {
            row[resultSet.fieldName(i)] = resultSet.field(i);
          }
          this.debug('Result:', row);
  ...
  TigerTable = (function(_super) {
    ...
    TigerTable.prototype.insert = function() {
      var args, cols, i, qStr, sendStr, _i, _j, _len, _len2, _ref;
  ...
  if (typeof module !== "undefined" && module !== null) module.exports = TigerDB;
