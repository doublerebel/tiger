...
(function() {
  ...
  TigerDB = (function(_super) {
    ...
    TigerDB.prototype.executeQuery = function(string, args, callback) {
      var arg, error, fields, i, rawValue, resultArray, resultSet, row, value, _i, _ref;
      ...
      resultArray = [];
      if (resultSet) {
        fields = Ti.Platform.osname === 'android' ? resultSet.fieldCount : resultSet.fieldCount();
        while (resultSet.isValidRow()) {
          row = {};
          for (i = _i = 0, _ref = fields - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
  ...
  TigerTable = (function(_super) {
    ...
    TigerTable.prototype.commit = function(attributes) {
    ...
    TigerTable.prototype.all = function() {
  ...
  Model.TigerDB = {
    ...
    install: function() {
      if (__indexOf.call(this.attributes, "id") < 0) {
    ...
    loadTigerDB: function(filter) {
      ...
      result = filter ? this.db[this.name].find(filter) : this.db[this.name].all();
      if (!result) {
        return result;
      } else {
